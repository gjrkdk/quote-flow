...
must_haves:
  truths:
    ...
    - "Tab key moves focus to next cell in grid (spreadsheet behavior per CONTEXT.md)"
...
<task type="auto">
  ...
  <action>
    ...
    - Tab: Move focus to next cell (right one column). If at last column, wrap to first cell of next row. If at last cell of grid, move focus OUT of grid to next page element. Call `e.preventDefault()` when moving within grid, allow default when exiting grid from last cell.
    - Shift+Tab: Move focus to previous cell (left one column). If at first column, wrap to last cell of previous row. If at first cell of grid, allow default Tab behavior to move to previous page element.
...
<success_criteria>
- Full keyboard navigation in matrix grid: arrow keys, Tab/Shift+Tab (next/prev cell), Enter, Escape, Home, End
