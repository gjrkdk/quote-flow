  ...
  <action>
    ...
    **UI Changes - CSV Upload Area:**
    ...
    - If store HAS paid plan:
      ...
      - On DropZone onDrop callback, read file client-side:
        1. Get file from `acceptedFiles[0]`
        2. Create `new FileReader()`
        3. Set `fileReader.onload = (e) => setCsvContent(e.target?.result as string)`
        4. Call `fileReader.readAsText(file)`
        5. Store content in component state: `const [csvContent, setCsvContent] = useState<string | null>(null)`
        6. When csvContent is set, show "Preview" button
        7. On Preview click, submit form with `intent="preview_csv"` and csvContent as a hidden `<input type="hidden" name="csvContent" value={csvContent} />`
    ...
    **Action Changes - Enforce Free Limit on ALL Templates:**
    - At the TOP of the action function, before the intent switch/if block, add the free-tier check for all creation intents:
      ```
      const formData = await request.formData();
      const intent = formData.get("intent") as string;

      // For any matrix creation intent, enforce free tier limit
      if (intent === "create" || intent === "confirm_csv") {
        const { session } = await authenticate.admin(request);
        const store = await prisma.store.findUnique({ where: { shopDomain: session.shop } });
        const matrixCount = await prisma.priceMatrix.count({ where: { storeId: store.id } });
        const canCreate = await canCreateMatrix(request, matrixCount);
        if (!canCreate.allowed) {
          return json({ error: "Free plan limit reached. Upgrade to create more matrices." }, { status: 403 });
        }
      }
      ```
    - This ensures Small, Medium, Custom (intent="create") and CSV (intent="confirm_csv") are ALL gated by the same check.
    ...
    **Form Handling:**
    ...
    Note: File is read client-side with FileReader (see DropZone onDrop above) and sent as hidden text field. No multipart handling needed.
  </action>
  <verify>
