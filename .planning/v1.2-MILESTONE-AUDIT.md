...
gaps:
  ...
  integration:
    - "Phase 15 GDPR webhooks: Async job queue built but deliberately disabled - reverted to synchronous processing due to Vercel Hobby plan constraints"
  flows: []
tech_debt:
  - phase: 15-graphql-migration-gdpr
    items:
      - "Job queue infrastructure exists but unused (job-queue.server.ts, api.cron.process-jobs.ts, vercel.json empty crons array)"
      - "Async GDPR processing disabled - synchronous processing acceptable for MVP (GDPR events rare)"
      - "Consider enabling when upgrading to Vercel Pro or scaling beyond MVP"
  - phase: global
    items:
...
**Audited:** 2026-02-13T11:35:00Z
**Status:** TECH_DEBT — All delivered requirements met, App Store submission requirements deferred
...
- ⚠ Phase 15: Async job queue built but deliberately disabled (sync processing)

**Issue:** Phase 15 delivered async job queue infrastructure (job-queue.server.ts, cron endpoint) but deliberately reverted to synchronous GDPR processing due to Vercel Hobby plan constraints. This is an acceptable MVP simplification, not a broken integration.
...
**GDPR Flow:** Webhook receives shop/redact → Process → Delete store with cascades
**Status:** ✓ SIMPLIFIED (synchronous processing, not async as planned)
...
### Phase 15: Async Infrastructure Unused

**Items:**
1. Job queue service exists but unused (job-queue.server.ts)
2. Cron endpoint exists but not scheduled (api.cron.process-jobs.ts)
3. Vercel.json has empty crons array
4. GDPR webhooks process synchronously instead of async

**Rationale:** Vercel Hobby plan constraint. GDPR events rare enough for sync processing.

**Impact:** Low — Core functionality unaffected, infrastructure ready for future activation.

**Resolution Path:**
- Option A: Upgrade to Vercel Pro and enable cron schedule
- Option B: Keep sync processing for MVP scale (acceptable)
- Option C: Migrate to external job queue service (Inngest, Quirrel, BullMQ)
...
**Status: SHIP v1.2 WITH TECH DEBT ACKNOWLEDGMENT**
...
✅ **GDPR Compliance:** Webhooks functional with synchronous processing
- shop/redact deletes store with cascade to all related data
- customers/redact acknowledges (no customer PII stored)
- Response time well under 200ms requirement
...
⚠️ **Async Job Processing:** Infrastructure built but disabled
- Job queue service exists but unused
- Synchronous GDPR processing acceptable for MVP
- Can be enabled when upgrading to Vercel Pro

### Ship Decision

**YES — Ship v1.2 with the following understanding:**

1. **Core option groups feature is production-ready** — All functional requirements met, all phases verified
2. **App Store submission is deferred to future milestone** — App works in production but not publicly listed
3. **Async processing is simplified for MVP** — Acceptable tradeoff for Vercel Hobby plan constraints
4. **Tech debt is documented and tracked** — Clear resolution paths identified
...
- Upgrade to Vercel Pro and enable async GDPR processing
- OR migrate to external job queue service
- OR keep sync processing if GDPR event volume stays low
